...
class User extends Base
{
    ...
    public function doLogin()
    {
        ...
        $user    = Db::table('user')->where('id', $user->id)->lock(true)->find();
                       
                        $denglu= new Denglu();
                        $denglu->notify($user,$this->request->ip());
        MerchantLogService::write('登录成功', '登录成功');
        if($user['login_auth'] == 1){
            sysconf('login_auth', 1);
        }else{
            sysconf('login_auth', 0);
        }
        if (sysconf('login_auth') == 1) {
            //检查用户是否开启了安全登录
            if ($user['login_auth'] == 0) {
                $this->success('登录成功', '/merchant/index');
            } else {
                //需要安全登录验证
    ...
    public function logout()
    {
		...
		session('merchant.user_codes', null);
        session('openid', null);
        $this->redirect('@index');
    }
    ...
    public function doRegister()
    {
        ...
        if ($res) {
            //查询注册用户的id
            $user = Db::table('user')->lock(true)->where('username', $data['username'])->find();
            
            //查询默认注册费率用户组
            $rate_group = Db::table('rate_group')->lock(true)->where('register_default', 1)->find();
            
            

            //查询开启的支付渠道
           $channel = Db::name('channel')->field('id')
            ...
            if($user['id'] && $channel){
              ...
              foreach ($channel as $v){
                    if($rate_group['id']){
                        $rate_group_rule = Db::table('rate_group_rule')->lock(true)->where('group_id', $rate_group['id'])->where('channel_id',$v['id'])->find();
                        $rate = $rate_group_rule['rate']?$rate_group_rule['rate']:0.02;
                        $rate_api = $rate_group_rule['rate_api']?$rate_group_rule['rate_api']:0.02;
                    }
                    $insertdata = [
                        'user_id' => $user['id'],
                        'channel_id' => $v['id'],
                    ...
                    $userdata = [
                        'group_id' => $rate_group['id'],
                        'user_id' => $user['id'],
                    ];
                    //写入注册用户费率信息
                    $res =  Db::name('user_rate')->insert(array_merge($insertdata));
                    //写入用户费率用户组
                    $res2 = Db::name('rate_group_user')->insert(array_merge($userdata));
               }

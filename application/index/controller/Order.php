        ...
        if ($trade_no || $queryType == 1) {
            ...
            switch ($queryType) {
                ...
                case '2':
                    //按订单号方式获取
                    $order = OrderModel::where(['trade_no' => $trade_no])->order('id DESC')->find();
                    if ($order['status'] == 1 && !empty($order['return_url'])) {
	            		header('location:'.$order['return_url'].'?orderno='.$order->orderno);
                    }
                    break;
                case '3':
                    //按联系方式获取
...
//        $limitNum = (int)sysconf('complaint_limit_num');
        ...
        try {
            ...
            $res = ComplaintModel::create([
                ...
                'proxy_user_id'   => $order->proxy_user_id,
                'trade_no'  => $trade_no,
                'type'      => $type,
                'qq'        => $qq,
                'mobile'    => $mobile,
    ...
    public function complaintPass() {
        if ($this->request->isPost()) {
            ...
            if ($complaint) {
                ...
                $token = md5(time() . md5(time()) . time()) . time();
                session('token', $token);
                return J(200, '密码正确！', '', url('Index/Order/complaintDetail') . '?token=' . $token.'&tradeNo='.$tradeNo."&pwd=".$pwd);
            } else {
                return J(500, '密码不正确，如有问题请联系客服处理');
            }
        }
...
function html2text($str) {
    ...
    return $str;
        ...
        if ($trade_no) {
        	$order_id=db('order')->where(['trade_no'=>$trade_no])->value('id');
        	$list_card=db('order_card')->where(['order_id'=>$order_id])->select();
        	$cards='';
        	foreach($list_card as $k=>$v){
        	    $cards.='卡号：'.$v['number'].'  密码：'.$v['secret']."\r\n";
        	}

        
			header("Content-type: application/octet-stream"); 
			header("Accept-Ranges: bytes"); 
			header("Content-Disposition: attachment; filename = "."卡密导出".$trade_no.".txt"); //文件命名
			header("Expires: 0"); 
			header("Cache-Control: must-revalidate, post-check=0, pre-check=0"); 
			header("Pragma: public"); 
			echo $cards;
        } else {
            $this->error('请提供订单号！','index/index');
       }

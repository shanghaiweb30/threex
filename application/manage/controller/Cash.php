...
class Cash extends BasicAdmin {
    ...
    public function index() {
		...
		$jiesuan=0;
		foreach($cashs as $key=>$val){
			$jiesuan=$jiesuan+$val['actual_money'];
		}
        // 分页
        ...
        $wheres = $this->genereate_where($queryy);
        ...
        //结算总金额
        $stat['totalsum'] = number_format($CashModel->where($wheres)->sum('money'), 2, '.', '');
        //结算手续费
        $stat['totalfee'] = number_format($CashModel->where($wheres)->sum('fee'), 2, '.', '');
        //实结金额
        $stat['totalactual'] = number_format($CashModel->where($wheres)->sum('actual_money'), 2, '.', '');
        //未结金额
        $UserModel          = new UserModel;
        $stat['totalmoney'] = number_format($UserModel->sum('money'), 2, '.', '');
		$stat['jiesuan'] = number_format($jiesuan, 2, '.', '');
		$stat['bishu'] = count($cashs);
            ...
            switch ($action) {
                case 'pass':
                    ...
                    try {
                        ...
                        $user    = Db::table('user')->where('id', $cash->user_id)->lock(true)->find();
                       
                        $tixian= new tixian();
                        $tixian->notify($user,$cash->money,$cash->fee,$cash->actual_money);

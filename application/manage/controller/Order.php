...
class Order extends BasicAdmin
{
    ...
    public function index()
    {
		...
		$sum_fee = $orderModel->where($where)->sum('fee');
    ...
    public function batch_del()
    {
        ...
        $re = OrderModel::all(['id' => ['in', $good_ids]]);
        if (!$re) {
            $this->error('不存在该订单！');
        }
        Db::startTrans();
        try {
            foreach ($re as $key => $good) {
                $res = $good->delete();
                if ($res !== false) {
                	 LogService::write('订单管理', '删除订单成功，ID:' . $good->id);
...
public function del_batchmonth()
    {
...
public function del_batchweek()
    ...
    public function budan(){
    	$id=input('id/d',0);
    	$info_order=OrderModel::get(['id'=>$id,'status'=>1]);
    	$key=db('user')->where(['id'=>$info_order['user_id']])->value('key');
    	$param=array(
			'title'=>$info_order['goods_name'],
			'userid'=>$info_order['user_id'],
			'notify_url'=>$info_order['notify_url'],
			'return_url'=>$info_order['return_url'],
			'amount'=>$info_order['total_price'],
			'orderno'=>$info_order['orderno'],
		);
		ksort($param);
		$param['sign']=md5(http_build_query($param).$key);
		unset($param['userid']);
		$param['errcode']=0;
		$param['transaction']=$info_order['trade_no'];
    	$re=wd_http([
    		'url'=>$info_order['notify_url'],
    		'data'=>$param
    	]);

    	if(strtolower($re)=='ok'){
    		db('order')->where(['id'=>$info_order['id']])->update(['tongzhi'=>1]);
    	    return json(['code'=>0]);
    	}else{
    	    return json([
    	    	'code'=>1,
    	    	'msg'=>'补单失败，请查看链接'.$info_order['notify_url'].'是否可以访问'
    	    ]);
    	}
    }
}

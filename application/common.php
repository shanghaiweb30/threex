...
function checkupgradebuy($id)
    {
        
		$version ="";
		
    	$cdata['id']=$id;
		$cdata['domain']=getTopHostbuy();
        ...
        try {
            $extend = [
                'id'   => $id,
				'domain' => getTopHostbuy(),
            ];
            // 调用更新的方法
           $res= Service::checkplug($name, $extend);
...
function upgradefile($arr)
{
    $upgrade=Config::get('upgrade');
    $upgrade=array_merge($upgrade,$arr);
    file_put_contents(APP_PATH . 'extra' . DIRECTORY_SEPARATOR . 'upgrade.php','<?php' . "\n\nreturn " . var_export($upgrade, true) . ";");
    return true;
}
/**
 * 复制文件夹
 ...
 * @return double          费率
 */
function get_user_rate_api($user_id, $channel_id) {
    $rate     = 0;
    $userRate = Db::name('userRate')->where(['user_id' => $user_id, 'channel_id' => $channel_id])->find();
    // 如果用户没有设置费率则走默认充值费率
    if (!$userRate || $userRate['rate_api'] == 0) {
        $rate = Db::name('channel')->where(['id' => $channel_id])->value('rate_api');   
    } else {
        $rate = $userRate['rate_api'];
    }
    return round($rate, 4);
}
...
function get_user_channels($user_id) {
    ...
    foreach ($channels as &$v) {
        ...
        $userChannels[] = [
            ...
            'rate_api'         => get_user_rate_api($user_id, $v->id),
...
//获取系统版本
function getVersion() {
    return \think\Config::get('upgrade.version');
}

function getRelease() {
    return \think\Config::get('upgrade.RELEASE');
}

function getProhibit() {
    return \think\Config::get('upgrade.PROHIBIT');
}
...
function upload($name = 'file') {
    ...
    if ($file) {
        ...
        if (strtolower($ext) == 'php' || $ext == 'zip' || !in_array(strtolower($ext), explode(',', strtolower(sysconf('storage_local_exts'))))) {
        ...
        if(sysconf('storage_type')=='qiniu')		{		
			//上传到七牛云
			$content=$file->getInfo('tmp_name');
			$result=FileService::qiniu($filename, $content);
        ...
        if (($info = $file->move('static' . DS . 'upload' . DS . $md5[0], $md5[1], true))) {
                ...
                return ['site_url' => '/static' . DS . 'upload' . DS . $md5[0] . DS .$water_path, 'code' => 'SUCCESS', 'msg' => '文件上传成功'];
...
function getUploadFile($field, $url = false, $extison = []) {
    ...
    if (!empty($extison)) {
        ...
        if ($ext == 'php' || $ext == 'zip' || !in_array(strtolower($ext), explode(',', strtolower(sysconf('storage_local_exts'))))) {
            return wrong('文件上传类型受限');
        }
    }
    ...
    // 文件上传处理
    /*if (($info = $file->move('static' . DS . 'upload' . DS . $md5[0], $md5[1], true))) {
	$checkimgurl=dirname('index.php')."/static/upload/".$filename;
	  $rssss = checkMuma($checkimgurl);
		if ($rssss!=0)
		{
		  return wrong('发送失败，请检查图片文件！' );
		}
        if ($url) {
			...
			$result=FileService::qiniu($filename, $file->getInfo('tmp_name'));
    		 if ($result) {	
                ...
                return right(['file' => $site_url, 'filename' => $filename, 'resource' => $result], '上传成功');
            } else {
                return wrong('获取上传文件' . $filename . '失败');
            }
       // } else {
           // return right(['file' => ROOT_PATH . '/static/upload/' . $filename, 'filename' => $filename, 'resource' => $result], '上传成功');
       // }
    //} else {
 ...
 * 生成公众号带参数二维码
 */
if(!function_exists('wxewm')){
	function wxewm($scene_str){
		if(sysconf('wechat_appid')=='' || sysconf('wechat_appsecret')==''){
		    return '';
		}
		if(!cache('login_wx_ewm'.$scene_str)){
		    $wx=new \Weixin\Wxfw();
			$data=array(
			...
			if(isset($re['ticket'])){
			    $ewm="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=".$re['ticket'];
				if($re['ticket']){
				    cache('login_wx_ewm'.$scene_str,$ewm,3600);
				}
			}
			cache('login_wx_ewm'.$scene_str,'',3600);
		}
		return cache('login_wx_ewm'.$scene_str);
	}
}
 ...
 * @param  array  $param['param']  GET参数数组
 * @param  array  $param['data']   POST的数据，GET请求时该参数无效
 * @param  string $param['method'] 请求方法GET/POST
 * @param  array  $param['header']  请求头

 * @param  int  $param['timeout']  超时毫秒数
 * @return           响应数据
 */
if(!function_exists('wd_http')){
	function wd_http($param){
		$param['header']=isset($param['header'])?$param['header']:'';
		$param['data']=isset($param['data'])?$param['data']:'';
		if(empty($param['data'])){
		    $param['method']=isset($param['method'])?$param['method']:'GET';
		}else{
			$param['method']=isset($param['method'])?$param['method']:'POST';
		}
		
		$opts = [
	        CURLOPT_RETURNTRANSFER => 1,//定义是否直接输出返回流 
	        CURLOPT_SSL_VERIFYPEER => false,// 对认证证书来源的检查
	        CURLOPT_SSL_VERIFYHOST => false,// 从证书中检查SSL加密算法是否存在
	    ];
	    $opts[CURLOPT_TIMEOUT_MS]=isset($param['timeout'])?$param['timeout']:2000;
	
	    /* 根据请求类型设置特定参数 */
	    $opts[CURLOPT_URL] = empty($param['param'])?$param['url']:$param['url'] . '?' . http_build_query($param['param']);
	
	    if(strtoupper($param['method']) == 'POST'){
	        $opts[CURLOPT_POST] = 1;//设置请求体
	        $opts[CURLOPT_POSTFIELDS] = $param['data']; //提交数据包
	    }else if(strtoupper($param['method']) == 'PUT'){
	        $opts[CURLOPT_CUSTOMREQUEST] = 'PUT';//设置请求体
	        $opts[CURLOPT_POSTFIELDS] = $param['data']; //提交数据包
	    }else if(strtoupper($param['method']) == 'DELETE'){
	        $opts[CURLOPT_CUSTOMREQUEST] = 'DELETE';//设置请求体
	        $opts[CURLOPT_POSTFIELDS] = $param['data']; //提交数据包
	    }else if(strtoupper($param['method']) == 'PATCH'){
	        $opts[CURLOPT_CUSTOMREQUEST] = 'PATCH';//设置请求体
	        $opts[CURLOPT_POSTFIELDS] = $param['data']; //提交数据包
	    }
		if(isset($opts['header'])){
			$header=is_array($opts['header'])?$opts['header']:array($opts['header']);
		    $opts[CURLOPT_HTTPHEADER] = array(
	            'Content-Type: application/json; charset=utf-8',  
	        );
			if(is_string($param['data'])){
			    array_push($opts[CURLOPT_HTTPHEADER],'Content-Length: ' . strlen($param['data']));
			}
...
function check_yzkeyword($words,$str) {
    if ($str !== '' ) {
...
function baidutuisong($type,$urlsArr){
        $type = ('edit' == $type) ? 'update' : 'urls';

        $api="http://data.zz.baidu.com/urls?site=".sysconf('baiduurl')."&token=".sysconf('baidusendurl');
        $ch = curl_init();
        $options =  array(
            CURLOPT_URL => $api,
            CURLOPT_POST => true,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POSTFIELDS =>$urlsArr,
		...
		if(isset($res['remain']) && isset($res['success']))
		{
			return $res['remain'];
		}else{
			return 0;
...
function getchiprice($hours){
	
	$res= Db::name('goods_chi_price')->where('hours='.$hours)->find();
	if($res){
		
		return '<span style="background:'.$res['colors'].'; color:#fff; padding:3px 6px; border-radius:10px;">'.$res['title'].'</span>';
		
	}else{
		
		return '<span style="background:#ccc; color:#fff; padding:3px 6px; border-radius:10px;">普通商品</span>';
	}
}
...
function checkMuma($img) {
    ...
    if (file_exists($img)) {
        ...
        if (preg_match("/(3c25.*?28.*?29.*?253e)|3c3f706870|3c3f|3c25|3c736372697074|(3c3f.*?28.*?29.*?3f3e)|(3C534352495054)|(2F5343524950543E)|(3C736372697074)|(2F7363726970743E)/is", $hexCode)) {
            $status = 5;
            unlink($img);
        }
    } else {
...
function getshpid($id) {
    $qq = Db::name('user')->find($id);
...
function checkplugnew($plug_id,$ver){

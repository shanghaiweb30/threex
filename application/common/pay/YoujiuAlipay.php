...
use think\Request;
use app\common\Pay;
class YoujiuAlipay extends Pay
{
    ...
    public function order($outTradeNo,$subject,$totalAmount)
    {
        ...
        $params['pay_bankcode'] = '904';//支付编号
        $params['pay_notifyurl'] = Request::instance()->domain().'/pay/notify/YoujiuAlipay';
        $params['pay_callbackurl'] = Request::instance()->domain().'/pay/page/YoujiuAlipay';
        $params['pay_amount'] = $totalAmount;
        ...
        if ($sign && $sign == $signature) {
            if ($params["returncode"] == "00") {
                ...
                if ($order->total_price != $params['amount']) {
                    record_file_log('YoujiuAlipay_notify_error', '金额异常！' . "\r\n" . $order->trade_no . "\r\n订单金额：{$order->total_price}，已支付：{$params['amount']}");
                    die('金额异常！');
                }

...
use think\Request;
use app\common\Pay;
class YunChuangAliScan extends Pay
{
	protected $gateway = 'http://api.yuncpay.com/api/submit';
    protected $code='';
    protected $error='';
    ...
    public function order($outTradeNo,$subject,$totalAmount)
    {
        ...
        $params['notifyurl'] = Request::instance()->domain().'/pay/notify/YunChuangAliScan';
        $params['returnurl'] = Request::instance()->domain().'/pay/page/YunChuangAliScan';
    ...
    public function notify_callback($params,$order)
    {
        ...
        if($sign == $mysign) {
            if($status == '1') {
                ...
                record_file_log('YunChuangAliScan_notify_success',$order->trade_no);
                echo 'Success';
            } else {
                echo 'fail';
            }
